...
sub update_or_create_transit_map_layer   { goto &update_or_create_gtfs_map_layer;   }
sub update_or_create_transit_stops_layer { goto &update_or_create_gtfs_stops_layer; }
sub get_transit_routes                   { goto &get_gtfs_routes;                   }
sub get_transit_stops                    { goto &get_gtfs_stops;                    }
sub get_excepted_transit_trips           { goto &get_excepted_gtfs_trips;           }
sub get_transit_shape_ids_from_trip_ids  { goto &get_gtfs_shape_ids_from_trip_ids;  }
sub get_transit_route_shape_ids          { goto &get_gtfs_route_shape_ids;          }
sub get_transit_route_shape_points       { goto &get_gtfs_route_shape_points;       }
sub clear_transit_map_layers             { goto &clear_gtfs_map_layers;             }
sub clear_transit_stops_layers           { goto &clear_gtfs_stops_layers;           }
sub draw_transit_stops                   { goto &draw_gtfs_stops;                   }
sub draw_transit_routes                  { goto &draw_gtfs_routes;                  }

sub update_or_create_gtfs_map_layer {
...
sub update_or_create_gtfs_stops_layer {
    my ($self, $map_area, $map_area_layer) = @_;
    $self->{_dirty_} = 1;
...
sub get_gtfs_shape_ids_from_trip_ids {
    my ($self, %args) = @_;
    my $gtfs  = $args{gtfs};
    my @trips = @{$args{trips}};
...
sub get_gtfs_route_shape_points {
...
sub clear_gtfs_map_layers {
    my ($self) = @_;
    $self->{_dirty_} = 1;
    foreach my $map_area (@{$self->{_map_areas}}) {
	my $map_area_layer = $self->update_or_create_map_area_layer($map_area);
	my $transit_map_layer = $self->update_or_create_gtfs_map_layer($map_area, $map_area_layer);
	$self->erase_autogenerated_content_within($transit_map_layer);
    }
}

sub clear_gtfs_stops_layers {
    my ($self) = @_;
    $self->{_dirty_} = 1;
    foreach my $map_area (@{$self->{_map_areas}}) {
	my $map_area_layer = $self->update_or_create_map_area_layer($map_area);
	my $transit_stops_layer = $self->update_or_create_gtfs_stops_layer($map_area, $map_area_layer);
...
sub draw_gtfs_stops {
	...
	foreach my $map_area (@{$self->{_map_areas}}) {
		...
		my ($x, $y) = $self->lon_lat_deg_to_svg($lon_deg, $lat_deg);
...
sub draw_gtfs_routes {
	...
	foreach my $route ($self->get_gtfs_routes($gtfs)) {
	    ...
	    my @shape_id = $self->get_gtfs_route_shape_ids($gtfs, $route_short_name);
	    ...
	    my @excepted_trips = $self->get_excepted_gtfs_trips(gtfs => $gtfs, route => $route_short_name);
	    my @excluded_trips = $self->get_excepted_gtfs_trips(gtfs => $gtfs, route => $route_short_name, return_excluded_trips => 1);
	    my @excepted_shape_id = $self->get_gtfs_shape_ids_from_trip_ids(gtfs => $gtfs, trips => \@excepted_trips);
	    my @excluded_shape_id = $self->get_gtfs_shape_ids_from_trip_ids(gtfs => $gtfs, trips => \@excluded_trips);
		...
		foreach my $shape_id (@shape_id) {
		    my @coords = $self->get_gtfs_route_shape_points($gtfs, $shape_id);
		    $shape_coords{$agency_route}{$shape_id} = [@coords];
		    my @svg_coords = map {
			my ($svgx, $svgy) = $self->lon_lat_deg_to_svg($_->[0], $_->[1]);

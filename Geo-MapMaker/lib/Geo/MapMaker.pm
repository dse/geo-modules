...
sub update_or_create_background_layer {
    ...
    my $rect = $self->rectangle(x      => $self->west_map_boundary_svg,
				y      => $self->north_map_boundary_svg,
				width  => $self->east_map_boundary_svg - $self->west_map_boundary_svg,
				height => $self->south_map_boundary_svg - $self->north_map_boundary_svg,
...
sub draw_transit_stops {
	...
	foreach my $map_area (@{$self->{_map_areas}}) {
	    $self->update_scale($map_area);
	    my $west_svg  = $self->west_map_boundary_svg;
	    my $east_svg  = $self->east_map_boundary_svg;
	    my $north_svg = $self->north_map_boundary_svg;
	    my $south_svg = $self->south_map_boundary_svg;
...
sub draw_transit_routes {
	...
	foreach my $route ($self->get_transit_routes($gtfs)) {
		...
		foreach my $shape_id (@shape_id) {
		    ...
		    my @svg_coords = map {
			my $svgx = $self->{converter}->lon_deg_to_x_px($_->[0]);
			my $svgy = $self->{converter}->lat_deg_to_y_px($_->[1]);
...
sub draw_openstreetmap_maps {
	...
	foreach my $map_area (@{$self->{_map_areas}}) {
		...
		my $svgx = $self->{converter}->lon_deg_to_x_px($lon_deg);
		my $svgy = $self->{converter}->lat_deg_to_y_px($lat_deg);
		my $xzone = ($svgx <= $west_svg)  ? -1 : ($svgx >= $east_svg)  ? 1 : 0;
		my $yzone = ($svgy <= $north_svg) ? -1 : ($svgy >= $south_svg) ? 1 : 0;
...
sub draw_crop_lines {
    ...
    my $south_deg = $self->south_map_boundary_deg; my $y_south = $self->{converter}->lat_deg_to_y_px($south_deg);
    my $north_deg = $self->north_map_boundary_deg; my $y_north = $self->{converter}->lat_deg_to_y_px($north_deg);
    my $east_deg  = $self->east_map_boundary_deg;  my $x_east = $self->{converter}->lon_deg_to_x_px($east_deg);
    my $west_deg  = $self->west_map_boundary_deg;  my $x_west = $self->{converter}->lon_deg_to_x_px($west_deg);
...
sub west_map_boundary_svg {
    my ($self) = @_;
    return $self->{_west_svg};
}
sub east_map_boundary_svg {
    my ($self) = @_;
    return $self->{_east_svg};
}
sub north_map_boundary_svg {
    my ($self) = @_;
    return $self->{_north_svg};
}
sub south_map_boundary_svg {
    my ($self) = @_;
    return $self->{_south_svg};
}

sub west_map_boundary_deg {
    my ($self) = @_;
    return $self->{west_deg};
}
sub east_map_boundary_deg {
    my ($self) = @_;
    return $self->{east_deg};
}
sub north_map_boundary_deg {
    my ($self) = @_;
    return $self->{north_deg};
}
sub south_map_boundary_deg {
    my ($self) = @_;
...
BEGIN {
	...
	if (defined $self->{west_deg} &&
	      defined $self->{east_deg} &&
		defined $self->{north_deg} &&
		  defined $self->{south_deg}) {
	    ...
	    $converter->set_lon_lat_boundaries($self->{west_deg}, $self->{east_deg},

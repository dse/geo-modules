...
sub erase_autogenerated_clip_paths {
	...
	my @cpnodes =
		$doc->findnodes("//svg:defs/svg:clipPath[\@mapmaker:autogenerated]");
	foreach my $cpnode (@cpnodes) {
		warn("erasing autogenerated clip path.\n");
		$cpnode->unbindNode();
	}
}
...
sub erase_autogenerated {
	my ($self, $element) = @_;
	foreach my $child ($element->childNodes()) {
		if ($child->hasAttributes()) {
			if ($child->getAttributeNS($NS{"mapmaker"}, "autogenerated")) {
				$self->erase_autogenerated($child);
			}
			if (!scalar($child->nonBlankChildNodes())) {
				$child->unbindNode();
			}
		}
	}
...
sub map_area_layer {
	...
	my $map_area_layer = $self->layer(%$options,
					  name            => $layer_name,
					  writable        => 1,
					  parent          => $doc_elt,
...
sub stuff_all_layers_need {
	...
	foreach my $map_area (@{$self->{_map_areas}}) {
		...
		$self->white_layer($map_area_under_layer);
		$self->background_layer($map_area_under_layer);
		$self->style_node();
...
sub create_element {
	...
	my $not_autogenerated = $options && $options->{not_autogenerated};
	my $prefix;
	if ($name =~ m{:}) {
		$prefix = $`;
		$name = $';

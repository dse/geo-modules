...
sub _upgrade_mapmaker_version_from_1_to_2 {
	my ($self) = @_;

	# set mapmaker:children-autogenerated on certain mapmaker:autogenerated elements.
	#   anything that normally has insensitive = false

	my $defs = $self->defs_node();
	my $doc = $self->{_svg_doc};

	foreach my $style ($defs->findnodes("svg:style[\@mapmaker:autogenerated]")) {
		$style->setAttributeNS($NS{"mapmaker"}, "children-autogenerated", "true");
	}
	foreach my $map_area (@{$self->{_map_areas}}) {
...
sub stuff_all_layers_need {
	...
	foreach my $map_area (@{$self->{_map_areas}}) {
		$self->update_scale($map_area);
		my $map_area_under_layer = $self->map_area_layer($map_area, { under => 1 });
		my $map_area_layer = $self->map_area_layer($map_area);
...
sub update_or_create_layer {
	...
	my $layer;
	if (!$layer && defined $id) {
		($layer) = $parent->findnodes("svg:g[\@id='$id']");
	}
	if (!$layer && defined $name) {
		($layer) = $parent->findnodes("svg:g[\@inkscape:label='$name']");
	}
	if (!$layer && !$args{no_create}) {
	...
	if ($layer) {
		...
		if (defined $name) {
			$layer->setAttributeNS($NS{"inkscape"}, "inkscape:label", $name);
		} else {
			$layer->removeAttributeNS($NS{"inkscape"}, "inkscape:label");
		}
		if (defined $z_index) {
			$layer->setAttributeNS($NS{"mapmaker"}, "mapmaker:z-index", $z_index);
		} else {
			$layer->removeAttributeNS($NS{"mapmaker"}, "mapmaker:z-index");
		}
		$layer->setAttributeNS($NS{"sodipodi"}, "sodipodi:insensitive", $insensitive ? "true" : "false");
		if ($autogenerated) {
			$layer->setAttributeNS($NS{"mapmaker"}, "mapmaker:autogenerated", "true");
		} else {
			$layer->removeAttributeNS($NS{"mapmaker"}, "mapmaker:autogenerated");
		}
		if ($children_autogenerated) {
			$layer->setAttributeNS($NS{"mapmaker"}, "mapmaker:children-autogenerated", "true");
		} else {
			$layer->removeAttributeNS($NS{"mapmaker"}, "mapmaker:children-autogenerated");
		}
	}
	return $layer;
}
...
sub draw_crop_lines {
	...
	my $crop_lines_layer = $self->update_or_create_layer(name => "Crop Lines", z_index => 9996);
	...
	my $crop_marks_layer = $self->update_or_create_layer(name => "Crop Marks", z_index => 9997);

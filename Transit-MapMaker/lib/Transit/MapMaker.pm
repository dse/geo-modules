...
sub get_transit_routes {
	...
	my $dbh = $self->{_dbh} //= $gtfs->dbh();
	
	my $sth = $dbh->prepare_cached("select * from routes");
	$sth->execute();
	my @result = ();
...
sub get_transit_stops {
	...
	my $sth = $dbh->prepare_cached("select * from stops");
	$sth->execute();
	my @result = ();
	while (my $row = $sth->fetchrow_hashref()) {
		push(@result, { %$row });
...
sub get_excepted_transit_shape_ids {
	my ($self, @trips) = @_;
	my $gtfs = $self->{_gtfs};
	if (!$gtfs) { return; }
...
sub draw_transit_routes {
	...
	print STDERR ("Working on transit routes...\n") if $verbose;
	...
	foreach my $route ($self->get_transit_routes()) {
		...
		my @trips = $self->get_excepted_transit_trips($route_short_name);
		my @excepted_shape_id = $self->get_excepted_transit_shape_ids(@trips);
		my @shape_id = $self->get_transit_route_shape_ids($route_id);

		# FIXME: brute force for now
		my @non_excepted_shape_id = @shape_id;
		foreach my $shape_id (@excepted_shape_id) {
			@non_excepted_shape_id = grep { $_ ne $shape_id } @non_excepted_shape_id;
		}
		...
		foreach my $shape_id ($self->get_transit_route_shape_ids($route_id)) {
			my $path = [];
			foreach my $coords ($self->get_transit_route_shape_points($shape_id)) {
				my ($lon, $lat) = @$coords;
			...
			if (grep { $_ eq $shape_id } @excepted_shape_id) {
...
sub debug__document_size {
#	my ($self) = @_;
#	my ($package, $filename, $line) = caller();
#	my $length = length($self->{_svg_doc}->toString());
#	warn(sprintf("Document length is %10d at %s line %s\n", $length, $filename, $line));
}
